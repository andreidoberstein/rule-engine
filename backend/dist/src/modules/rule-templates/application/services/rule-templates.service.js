"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RuleTemplatesService = void 0;
const common_1 = require("@nestjs/common");
let RuleTemplatesService = class RuleTemplatesService {
    ruleTemplatesRepository;
    constructor(ruleTemplatesRepository) {
        this.ruleTemplatesRepository = ruleTemplatesRepository;
    }
    async create(createDto) {
        return this.ruleTemplatesRepository.create(createDto);
    }
    async findAll(cursor, take) {
        return this.ruleTemplatesRepository.findAll({ cursor, take });
    }
    async findOne(id) {
        const rule = await this.ruleTemplatesRepository.findById(id);
        if (!rule) {
            throw new common_1.NotFoundException(`Rule Template with ID ${id} not found`);
        }
        return rule;
    }
    async evaluateRules(clientId, stateUf) {
        return this.ruleTemplatesRepository.evaluateRules(clientId, stateUf);
    }
    async update(id, updateDto) {
        const rule = await this.ruleTemplatesRepository.findById(id);
        if (!rule) {
            throw new common_1.NotFoundException(`Rule Template with ID ${id} not found`);
        }
        return this.ruleTemplatesRepository.update(id, updateDto);
    }
    async remove(id) {
        const rule = await this.ruleTemplatesRepository.findById(id);
        if (!rule) {
            throw new common_1.NotFoundException(`Rule Template with ID ${id} not found`);
        }
        return this.ruleTemplatesRepository.delete(id);
    }
};
exports.RuleTemplatesService = RuleTemplatesService;
exports.RuleTemplatesService = RuleTemplatesService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)('IRuleTemplatesRepository')),
    __metadata("design:paramtypes", [Object])
], RuleTemplatesService);
//# sourceMappingURL=rule-templates.service.js.map