"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BudgetsModule = void 0;
const common_1 = require("@nestjs/common");
const budgets_service_1 = require("./application/services/budgets.service");
const budgets_controller_1 = require("./infrastructure/controllers/budgets.controller");
const prisma_budgets_repository_1 = require("./infrastructure/repositories/prisma-budgets.repository");
const prisma_module_1 = require("../../infrastructure/database/prisma/prisma.module");
const rule_templates_module_1 = require("../rule-templates/rule-templates.module");
let BudgetsModule = class BudgetsModule {
};
exports.BudgetsModule = BudgetsModule;
exports.BudgetsModule = BudgetsModule = __decorate([
    (0, common_1.Module)({
        imports: [prisma_module_1.PrismaModule, rule_templates_module_1.RuleTemplatesModule],
        controllers: [budgets_controller_1.BudgetsController],
        providers: [
            budgets_service_1.BudgetsService,
            {
                provide: 'IBudgetsRepository',
                useClass: prisma_budgets_repository_1.PrismaBudgetsRepository,
            },
        ],
        exports: [budgets_service_1.BudgetsService],
    })
], BudgetsModule);
//# sourceMappingURL=budgets.module.js.map